...
static void UpdateConfiguration(AsyncWebServerRequest *request, JsonVariant &json)
{
  if (json["button-actions"].is<JsonVariant>()) {
    JsonArray array = json["button-actions"].as<JsonArray>();
...
static void ImportConfiguration(AsyncWebServerRequest *request, JsonVariant &json)
{
  ...
  {
    JsonObject modelObj = json["model"].as<JsonObject>();
    for(JsonPair kv : modelObj)
    {
...
static void WebUdpControl(AsyncWebServerRequest *request)
{
  ...
  {
    // Get remote IP from request - using the client's remote address
    IPAddress remoteAddr = request->client()->getRemoteAddress();
    WifiJoystick::StartSending(remoteAddr,
