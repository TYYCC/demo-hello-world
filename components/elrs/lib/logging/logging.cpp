...
// Wrapper functions for ESP WiFi logging
extern "C" {
  int __wrap_esp_log_write(uint32_t level, const char* tag, const char* format, ...)
  {
    // Redirect WiFi logs to the debug stream if available
    if (BackpackOrLogStrm) {
      ...
      va_end(args);
      BackpackOrLogStrm->printf("[%s] %s", tag ? tag : "WIFI", buffer);
    }
    return 0;
  }

  int __wrap_esp_log_writev(uint32_t level, const char* tag, const char* format, va_list args)
  {
    // Redirect WiFi logs to the debug stream if available
    if (BackpackOrLogStrm) {
      char buffer[256];
