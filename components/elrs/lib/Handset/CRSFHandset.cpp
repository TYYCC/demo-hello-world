...
void CRSFHandset::duplex_set_RX() const
{
    ...
    if (UARTinverted)
    {
        esp_rom_gpio_connect_in_signal((gpio_num_t)GPIO_PIN_RCSIGNAL_RX, U0RXD_IN_IDX, true);
        gpio_pulldown_en((gpio_num_t)GPIO_PIN_RCSIGNAL_RX);
        gpio_pullup_dis((gpio_num_t)GPIO_PIN_RCSIGNAL_RX);
    }
    else
    {
        esp_rom_gpio_connect_in_signal((gpio_num_t)GPIO_PIN_RCSIGNAL_RX, U0RXD_IN_IDX, false);
...
void CRSFHandset::duplex_set_TX() const
{
    ...
    if (UARTinverted)
    {
        ...
        esp_rom_gpio_connect_in_signal(MATRIX_DETACH_IN_LOW, U0RXD_IN_IDX, false); // Disconnect RX from all pads
        esp_rom_gpio_connect_out_signal((gpio_num_t)GPIO_PIN_RCSIGNAL_TX, U0TXD_OUT_IDX, true, false);
    }
    else
    {
        ...
        esp_rom_gpio_connect_in_signal(MATRIX_DETACH_IN_HIGH, U0RXD_IN_IDX, false); // Disconnect RX from all pads
        esp_rom_gpio_connect_out_signal((gpio_num_t)GPIO_PIN_RCSIGNAL_TX, U0TXD_OUT_IDX, false, false);
    }
#elif defined(PLATFORM_ESP8266)
